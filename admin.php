      ...
      if (isset($_POST["import"])) {
        ...
        if ($_FILES["file"]["size"] > 0) {
          ...
          while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
            $madm = "";
            if (isset($column[0])) {
              $madm = mysqli_real_escape_string($conn, $column[0]);
            }
            $tensp = "";
            if (isset($column[1])) {
              $tensp = mysqli_real_escape_string($conn, $column[1]);
            }
            $gia = "";
            if (isset($column[2])) {
              $gia = mysqli_real_escape_string($conn, $column[2]);
            }
            $soluong = "";
            if (isset($column[3])) {
              $soluong = mysqli_real_escape_string($conn, $column[3]);
            }
            $hinhanh = "";
            if (isset($column[4])) {
              $hinhanh = mysqli_real_escape_string($conn, $column[4]);
            }
            $sqlInsert = "INSERT into sanpham (madm,tensp,gia,soluong,hinhanh)
                   values (?,?,?,?,?)";
            $paramType = "issss";
            $paramArray = array(
              $madm,
